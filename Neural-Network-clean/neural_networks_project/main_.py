...
# βάζεις εδώ τα threads ΠΡΙΝ το import numpy
...
def blacknwhite(x, mode="single"):
    """
    Convert CIFAR-10 flat RGB vectors (N, 3072) to grayscale.

    x   : np.ndarray of shape (N, 3072) or (3072,)
          first 1024: R, next 1024: G, last 1024: B
    mode: "single" -> returns (N, 1024)  (one gray channel)
          "stack3" -> returns (N, 3072) (gray repeated 3 times)
    ...
    # Handle single image (3072,) by adding batch dimension
    single_image = False
    if x.ndim == 1:
        assert x.shape[0] == 3072, "Expected shape (3072,) for a single image."
        x = x[np.newaxis, :]
        single_image = True

    assert x.ndim == 2 and x.shape[1] == 3072, "Expected shape (N, 3072)."

    # Split channels
    R = x[:, 0:1024].astype(np.float32)
    G = x[:, 1024:2048].astype(np.float32)
    B = x[:, 2048:3072].astype(np.float32)

    # Grayscale (luminance)
    gray = 0.299 * R + 0.587 * G + 0.114 * B  # shape (N, 1024)

    if mode == "single":
        out = gray
    elif mode == "stack3":
        out = np.concatenate([gray, gray, gray], axis=1)  # (N, 3072)
    else:
        raise ValueError('mode must be "single" or "stack3"')

    # Cast back to original dtype
    out = np.clip(out, 0, 255).astype(x.dtype)

    # If input was a single image, squeeze batch dimension
    if single_image:
        out = out[0]
    return out
...
def testmain():
    ...
    if BLACKNWHITE:
        data_train = blacknwhite(data_train)
    epoch = 3
    seasons = 50
    LR = 10**(-3)
    for e in range(epoch):
        ...
        if e != 0:
            time.sleep(5)
            WL1 = np.load(f"C:/Users/emili/Desktop/NeuralNetworks-Project-1/results/Weights_Layer1_season{e * seasons}.npy")
            WL2 = np.load(f"C:/Users/emili/Desktop/NeuralNetworks-Project-1/results/Weights_Layer2_season{e * seasons}.npy")
            WL3 = np.load(f"C:/Users/emili/Desktop/NeuralNetworks-Project-1/results/Weights_Layer3_season{e * seasons}.npy")
            WL4 = np.load(f"C:/Users/emili/Desktop/NeuralNetworks-Project-1/results/Weights_Layer4_season{e * seasons}.npy")
            WL5 = np.load(f"C:/Users/emili/Desktop/NeuralNetworks-Project-1/results/Weights_Layer5_season{e * seasons}.npy")
            #WL6 = np.load(f"C:/Users/emili/Desktop/NeuralNetworks-Project-1/results/Weights_Layer6_season{e * 10}.npy")
            bL1 = np.load(f"C:/Users/emili/Desktop/NeuralNetworks-Project-1/results/bias_Layer1_season{e * seasons}.npy")
            bL2 = np.load(f"C:/Users/emili/Desktop/NeuralNetworks-Project-1/results/bias_Layer2_season{e * seasons}.npy")
            bL3 = np.load(f"C:/Users/emili/Desktop/NeuralNetworks-Project-1/results/bias_Layer3_season{e * seasons}.npy")
            bL4 = np.load(f"C:/Users/emili/Desktop/NeuralNetworks-Project-1/results/bias_Layer4_season{e * seasons}.npy")
            bL5 = np.load(f"C:/Users/emili/Desktop/NeuralNetworks-Project-1/results/bias_Layer5_season{e * seasons}.npy")
        ...
        FC1 = FC_layer(Input=3072, Self=2048, batch=50,LR=LR, activation="relu", W=WL1, b=bL1)
        FC2 = FC_layer(Input=2048, Self=1024, batch=50, LR=LR, activation="relu", W=WL2, b=bL2)
        FC3 = FC_layer(Input=1024, Self=512, batch=50,LR=LR, activation="relu", W=WL3, b=bL3)
        FC4 = FC_layer(Input=512, Self=256, batch=50,LR=LR, activation="relu", W=WL4, b=bL4)
        FC5 = FC_layer(Input=256, Self=10, batch=50,LR=LR, activation="softmax", W=WL5, b=bL5)
        # FC6 = FC_layer(Input=256, Self=10, batch=50,LR=LR, activation="softmax", W=WL6, b=bL6)
        ...
        for i in range(seasons):
            ...
            for j in range(len(data_batch)):
                ...
                train(data_batch[j],labels_batch[j], FC1=FC1, FC2=FC2, FC3=FC3, FC4=FC4,FC5=FC5, FC6=FC6, FC7=FC7, dropout = False, keepProp=[0.7, 0.6, 0.6, 0.5, 0.5])
                print("Training ", end=" ")
...
def train(data_batch,labels_batch, FC1 = None, FC2 = None , FC3 = None, FC4 = None, FC5 = None, FC6 = None, FC7 = None, dropout = False, keepProp = [1, 1, 1, 1, 1]):
    batch = 50
    i = 0
    Los = []
    layers = [FC1, FC2, FC3, FC4, FC5, FC6, FC7]
    ...
    while i < len(data_batch): #Generate input and target
        ...
        for L in (active): #Forward Pass
            ...
            if (j < tot_layers - 1) and dropout:
                ...
                masks.append(rand_mat < keepProp[j])
                y = y * masks[j] / keepProp[j] 
                j += 1
        ...
        for L in reversed(active):#Bacckward Pass
            ...
            if dropout and mask_idx >= 0:
                grad = grad * masks[mask_idx] / keepProp[mask_idx]
                mask_idx -= 1
        ...
        if i % 10000 == 0 or i == 5 :
            ...
            print(f"Loss = { np.mean(Los):.6f}.", end=" ")
...
def shuffle():
    ...
    if BLACKNWHITE:
        data_batch_1 = blacknwhite(data_batch_1)
        data_batch_2 = blacknwhite(data_batch_2)
        data_batch_3 = blacknwhite(data_batch_3)
        data_batch_4 = blacknwhite(data_batch_4)
        data_batch_5 = blacknwhite(data_batch_5)
    if FLIP:
    ...
    an, bn = merge_shuffle_split([data_batch_1, data_batch_2, data_batch_3, data_batch_4, data_batch_5], [labels_batch_1, labels_batch_2, labels_batch_3, labels_batch_4, labels_batch_5])
    d1, d2, d3, d4, d5= an
    l1, l2, l3, l4, l5 = bn
    return [d1, d2, d3, d4, d5], [l1, l2, l3, l4, l5]
